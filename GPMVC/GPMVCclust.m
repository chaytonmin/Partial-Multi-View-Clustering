...
function [U ,Paired,result,objValue] = PVCclust(Xpaired, Xsingle, numClust, truth, options)
...
W=cell(views,1);
parfor v=1:views
   W{v}=constructW_cai([Xpaired{v};Xsingle{v}],options);
end
%W1 = constructW_cai([X1';X2'],options);
%W2 = constructW_cai([Y2';Y3'],options);

  %[Ux,Uy,P2,P1,P3,objValue] = GPVC(X2,Y2,X1,Y3,W1,W2,options);
 [U, Paired,P,objValue]=GPMVC(Xpaired,Xsingle, W,views,options);  %At this step we have our matrices, now perform stat collection
...
result = ComputeStats(PIn, truth, numClust);
